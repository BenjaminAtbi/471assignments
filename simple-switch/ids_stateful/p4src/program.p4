...
control IDS_Ingress(inout headers hdr,
    ...
    register<bit<FLOW_BIT_WIDTH>>(FLOW_ENTRIES) patterns;
    ...
    action _build_pattern() {
        ...
        patterns.read(meta.pattern, meta.flowindex);

        meta.pattern = meta.pattern[FLOW_BIT_WIDTH-PATTERN_WIDTH-1:0] ++ hdr.payload.data;
        // patterns.write(meta.flowindex, meta.pattern);
        patterns.write(meta.flowindex, meta.pattern);
    }
